...
class CalculatorTools:
    ...
    def eval_ast(node):
        ...
        if isinstance(node, ast.Expression):
            return CalculatorTools.eval_ast(node.body)

        elif isinstance(node, ast.Num):
            return node.n

        elif isinstance(node, ast.BinOp):
            left = CalculatorTools.eval_ast(node.left)
            right = CalculatorTools.eval_ast(node.right)
            op_type = type(node.op)

            if op_type not in CalculatorTools.allowed_operators:
                raise ValueError(f"不支持的运算符: {node.op}")

            return CalculatorTools.allowed_operators[op_type](left, right)

        elif isinstance(node, ast.UnaryOp):
            operand = CalculatorTools.eval_ast(node.operand)
            ...
            if node.id not in CalculatorTools.allowed_functions:
                raise ValueError(f"不支持的变量或函数: {node.id}")
            return CalculatorTools.allowed_functions[node.id]

        elif isinstance(node, ast.Call):
            func = CalculatorTools.eval_ast(node.func)
            args = [CalculatorTools.eval_ast(arg) for arg in node.args]
...
@tool
async def calculate(expression: str) -> Union[str, Dict[str, Any]]:
    """
    ...
    try:
        ...
        # 安全计算表达式
        result = CalculatorTools.eval_ast(tree.body)
...
if __name__ == "__main__":
    try:
        ...
        for idx, tool in enumerate(tools):
            ...
            print(f"  - 是否有arun方法：{hasattr(tool, 'arun')}")  # 异步工具需有arun方法

        # 2. 测试get_tools_map()
        tools_map = get_tools_map()
        print(f"\n[验证] 工具映射键：{list(tools_map.keys())}")
        print(f"[验证] 是否包含'metric_query'：{'metric_query' in tools_map}")
        print(f"[验证] 是否包含'calculate'：{'calculate' in tools_map}")
